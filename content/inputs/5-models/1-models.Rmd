...
The geometry (and topology) of an object is built using components (see the [opf documentation](content/input/4-scene/2-opf.html)). For example a simple coffee tree can be built using two components: leaves and metamers (for the woody organs). Each component can have its own models and parameters. For example we know that metamers won't make any photosynthesis, so they don't have any photosynthesis model. 
...
{{%attachments pattern=\"plant_coffee.yml\" /%}}
