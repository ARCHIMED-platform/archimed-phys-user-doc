...
In ARCIHMED, an object is defined by: 
...
The `Type` parameter defines the component types used to build the object (e.g. leaves and branches components are used to build a plant). The names of the components listed here should match the same names given in the `.opf` file. In the example given earlier in the [organization chapter](../../1-organisation/), the `.opf` file for a coffee tree gives two different components: leaf and metamer. A model file adapted for this `.opf` will list those components as follows:
...
{{% notice warning %}}
The name of the Type should match exactly the names given in the `.opf` file
{{% /notice %}}
