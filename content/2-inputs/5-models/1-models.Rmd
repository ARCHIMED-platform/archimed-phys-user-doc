...
The `Type` parameter defines the component types used to build the object (e.g. leaves and branches components are used to build a plant). The names of the components listed here should match the same names given in the `topology` node of the `.opf` file (see [the documentation about the opf files](../../4-scene/2-opf/)). In the example given earlier in the [organization chapter](../../1-organisation/), the `.opf` file for a coffee tree gives two different components: `Leaf` and `Metamer` (read internode here). A model file adapted for this `.opf` will list those components as follows:
