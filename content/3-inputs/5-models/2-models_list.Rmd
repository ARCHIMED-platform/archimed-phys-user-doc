...
There are two three different models available in ARCHIMED to compute their light interception and scattering: `Translucent`, `ignore` and `VirtualSensor`.
...
This model has two set of parameters : 
  ...
  * optical_properties. Used to parametrize the scattering factors for each waveband to compute how much light is [absorbed](https://en.wikipedia.org/wiki/Absorptance), [reflected](https://en.wikipedia.org/wiki/Reflectance) and [transmitted](https://en.wikipedia.org/wiki/Transmittance) by the component (*i.e.* a range of wavelength).
  ...
  But in ARCHIMED we consider the reflectance coefficient equal to the transmittance coefficient ($\rho = \tau$), so we use only one value to parameterize the model: the scattering factor ($\sigma$). It is the sum of $\rho$ and $\tau$, such as $\sigma = \rho + \tau$.
...
There are two models to compute the photosynthesis in ARCHIMED: `FarquharEnBalance` and `NRH`.
